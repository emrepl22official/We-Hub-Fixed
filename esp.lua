...
if not Degrade then
	...
	warn("emrepl22official_ESP > Your exploit does not support this module's optimizations! The visuals might be laggy and decrease performance.")
end
]=]
...
if emrepl22officialDeveloperESP and emrepl22officialDeveloperESP.Exit then
	emrepl22officialDeveloperESP:Exit()
end

--// Settings

getgenv().emrepl22officialDeveloperESP = {
	DeveloperSettings = {
		Path = "emrepl22official Developer/emrepl22official ESP/Configuration.cfg",
...
local Environment = getgenv().emrepl22officialDeveloperESP
...
local CoreFunctions = {
	...
	GenerateHash = function(Bits)
		...
		for _ = 1, Bits do
			Result ..= ("emrepl22official_ESP")[mathrandom(1, 2) == 1 and "upper" or "lower"](stringchar(mathrandom(97, 122)))
		end
...
local UtilityFunctions = {
	...
	WrapObject = function(self, Object, PseudoName, Allowed, RenderDistance)
		assert(self, "emrepl22official_ESP > UtilityFunctions.WrapObject - Internal error, unassigned parameter \"self\".")
		...
		if not Entry.IsAPlayer then
			if not pcall(function()
				...
				warn("emrepl22official_ESP > UtilityFunctions.WrapObject - Attempted to wrap object of an unsupported class type: \""..(__index(Entry.Object, "ClassName") or "N / A").."\"")
...
Environment.UnwrapAll = function(self) -- METHOD | (<void>) => <void>
	assert(self, "emrepl22official_ESP.UnwrapAll: Missing parameter #1 \"self\" <table>.")
...
Environment.Restart = function(self) -- METHOD | (<void>) => <void>
	assert(self, "emrepl22official_ESP.Restart: Missing parameter #1 \"self\" <table>.")
...
Environment.Exit = function(self) -- METHOD | (<void>) => <void>
	assert(self, "emrepl22official_ESP.Exit: Missing parameter #1 \"self\" <table>.")
	...
	if self:UnwrapAll() then
		...
		for Index, _ in next, Environment do
			getgenv().emrepl22officialDeveloperESP[Index] = nil
		end
		...
		getgenv().emrepl22officialDeveloperESP = nil
	end
end
...
Environment.UpdateConfiguration = function(DeveloperSettings, Settings, Properties) -- (<table> DeveloperSettings, <table> Settings, <table> Properties) => <table> New Environment
	assert(DeveloperSettings, "emrepl22official_ESP.UpdateConfiguration: Missing parameter #1 \"DeveloperSettings\" <table>.")
	assert(Settings, "emrepl22official_ESP.UpdateConfiguration: Missing parameter #2 \"Settings\" <table>.")
	assert(Properties, "emrepl22official_ESP.UpdateConfiguration: Missing parameter #3 \"Properties\" <table>.")

	getgenv().emrepl22officialDeveloperESP.DeveloperSettings = DeveloperSettings
	getgenv().emrepl22officialDeveloperESP.Settings = Settings
	getgenv().emrepl22officialDeveloperESP.Properties = Properties

	Environment = getgenv().emrepl22officialDeveloperESP

	return Environment
end

Environment.LoadConfiguration = function(self) -- METHOD | (<void>) => <void>
	assert(self, "emrepl22official_ESP.LoadConfiguration: Missing parameter #1 \"self\" <table>.")
...
Environment.SaveConfiguration = function(self) -- METHOD | (<void>) => <void>
	assert(self, "emrepl22official_ESP.SaveConfiguration: Missing parameter #1 \"self\" <table>.")
